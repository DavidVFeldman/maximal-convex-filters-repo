...
The enumeration of strata in Corollary \ref{cor:strata} and the tables of Section \ref{sec:low} are not formalized. Section \ref{sec:space} is formalized in full, including Corollary \ref{cor:collapse} both elementarily and in terms of Mathlib's \texttt{T2Quotient}, the first clause of Theorem \ref{thm:hM2} as an equivalence, the bijection of its second clause, and Proposition \ref{prop:notcylinder}. An earlier form of that second clause asserted a homeomorphism; the formalization refuted it, and the negation is proved in the development as \texttt{ConvexFilter.\allowbreak Space.\allowbreak not\_boundary\_homeomorph}. Question \ref{qn:fibre} is open. In Section \ref{sec:alg}, Propositions \ref{prop:algebra} and \ref{prop:members}, Theorem \ref{thm:charsep} and Lemma \ref{lem:ridge} are formalized; Corollary \ref{cor:strictly}, which combines Lemma \ref{lem:ridge} with the openness of the image of $\RR^n$ in the quotient, is not. In Remark \ref{rem:generality} the positive claim is certified as described above, while the failure of Lemma \ref{lem:sep} in infinite dimensions is quoted from the literature and not proved here. Remark \ref{rem:extension} is not formalized: the saturation it uses is not available in Mathlib at the pinned revision.
